...
import { api, ApiError, bootstrap, isStaticMode } from './api';
...
export function App() {
  ...
  const current = payload?.file ? { ...payload.file, editable: payload.file.editable && !isStaticMode } : undefined;
  ...
  return <div className="app-shell">
    ...
    {!isStaticMode && collectOpen && <CollectModal onClose={() => setCollectOpen(false)} onDone={async (message, path) => { setCollectOpen(false); await refresh(); if (path) await openFile(path); notify(message); }}/>}
    {!isStaticMode && createOpen && <CreateModal onClose={() => setCreateOpen(false)} onDone={async (path) => { setCreateOpen(false); await refresh(); await openFile(path); setEditing(true); notify('wiki 页面已创建'); }}/>}
...
function MainView({ route, dashboard, tabs, files, payload, linkIndex, onOpen, onOpenTab, onEdit, onWikiLink, onCollect, onRefresh, notify, readOnly }: any) {
  if (route.kind === 'home') return <Home dashboard={dashboard} onOpen={onOpen} onOpenTab={onOpenTab}/>;
  if (route.kind === 'tab') return route.key === 'inbox' && !readOnly ? <InboxView files={files} onOpen={onOpen} onCollect={onCollect} onRefresh={onRefresh} notify={notify}/> : <TabList tab={tabs.find((t: TabInfo) => t.key === route.key)} files={files.filter((f: FileEntry) => tabMatches(f, route.key))} onOpen={onOpen}/>;
  if (!payload) return <Empty title="文件不存在" text="文件可能已被移动或删除。"/>;
  const { file, content, url } = payload;
  if (file.kind === 'md' && content !== undefined) return <article className="doc-page"><DocHeader file={{ ...file, editable: file.editable && !readOnly }} onEdit={readOnly ? undefined : onEdit}/><MarkdownView content={content} file={file} linkIndex={linkIndex} onWikiLink={onWikiLink}/></article>;
  ...
  if (file.kind === 'pdf') return <article className="doc-page wide"><DocHeader file={file}/><iframe className="pdf-reader" src={url} title={file.title}/><a className="download-link" href={url} download><Download size={14}/>若浏览器无法预览，请下载原件</a></article>;
